...
export function canonicalizeEngagementTerms(input: EngagementTermsInput): string {
  ...
  if (input.splitShareA !== undefined) payload.splitShareA = Number(input.splitShareA);
  if (input.splitShareB !== undefined) payload.splitShareB = Number(input.splitShareB);
